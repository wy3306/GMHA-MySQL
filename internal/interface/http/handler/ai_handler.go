...
func (h *AIHandler) capabilities(w http.ResponseWriter, r *http.Request) {
	...
	writeAI(w, map[string]any{
		...
		"intent_grounding": map[string]any{
			"ordered_selectors": []string{"第一个/第二个", "首个/最后一个", "倒数第 N 个", "1号/2号"},
			"entity_aliases":    []string{"ID", "名称", "IP"},
			"contextual_scope":  []string{"集群内机器", "会话中已验证的上一个目标"},
			"ambiguity_policy":  "候选不唯一或序号越界时不生成可执行计划",
			"server_rechecked":  true,
		},
		"conversation_memory": map[string]any{
			"session_scoped":            true,
			"recent_message_limit":      8,
			"recent_character_limit":    8000,
			"recent_token_budget":       2400,
			"memory_token_budget":       2400,
			"single_prompt_char_limit":  12000,
			"single_prompt_token_limit": 4000,
			"rolling_summary":           true,
			"summary_strategy":          "structured_snapshot",
			"summary_fields":            []string{"goals", "constraints", "decisions", "progress", "open_questions"},
			"full_history_retransmit":   false,
