...
import { computed, createApp, defineAsyncComponent, onMounted, onUnmounted, ref, watch } from 'vue/dist/vue.esm-bundler.js'
import './style.css'
import './manager.css'
...
import './upgrade.css'
import './cluster-rolling-upgrade.css'
...
const navGroups = [
  ...
  { title: '集群运维', icon: '◇', items: [
    ...
    { id: 'automation', icon: '⚙', label: '集群自动化' }
  ] },
  ...
  { title: 'AI 与自动化', icon: '✦', items: [
    { id: 'ai-automation', icon: '✦', label: 'AI 运维中心' }
  ] },
...
const UserManual = defineAsyncComponent(() => import('./documentation.js').then(module => module.UserManual))
const APIDocumentation = defineAsyncComponent(() => import('./documentation.js').then(module => module.APIDocumentation))
...
// rolling between versions. The server catalog replaces this fallback as soon
// as the performance API is available; until then every known core/machine
// metric renders an explicit empty card instead of disappearing.
const performanceFallbackCatalog = [
  ['mysql_qps', 'QPS', 'mysql', 'performance', '次/s', 'sum', '集群每秒查询吞吐'],
  ['mysql_tps', 'TPS', 'mysql', 'performance', '次/s', 'sum', '集群每秒事务吞吐'],
  ['mysql_threads_connected', '当前连接数', 'mysql', 'connection', '个', 'sum', '当前客户端连接总数'],
  ['mysql_threads_running', '当前运行线程数', 'mysql', 'connection', '个', 'sum', '正在执行工作的线程数'],
  ['mysql_active_connections', '活跃连接数', 'mysql', 'connection', '个', 'sum', '非 Sleep 状态连接数'],
  ['mysql_lock_wait_sessions', '锁等待会话数', 'mysql', 'performance', '个', 'sum', '正在等待行锁的会话数'],
  ['mysql_blocked_sessions', '被阻塞会话数', 'mysql', 'performance', '个', 'sum', '当前被其他事务阻塞的会话数'],
  ['mysql_metadata_lock_waits', '元数据锁等待数', 'mysql', 'performance', '个', 'sum', '等待元数据锁的会话数'],
  ['mysql_active_transactions', '活跃事务数', 'mysql', 'performance', '个', 'sum', '当前未结束事务数'],
  ['mysql_longest_transaction_seconds', '事务最长运行时间', 'mysql', 'performance', 's', 'max', '当前最长事务持续时间'],
  ['mysql_slow_queries_per_min', '慢查询每分钟新增数', 'mysql', 'performance', '次/min', 'sum', '每分钟新增慢查询数量'],
  ['mysql_replication_lag', '复制延迟', 'mysql', 'replication', 's', 'max', '复制链路最大延迟'],
  ['mysql_buffer_pool_hit_ratio', 'Buffer Pool 命中率', 'mysql', 'storage', '%', 'avg', 'InnoDB Buffer Pool 逻辑读命中率'],
  ['cpu_usage_percent', 'CPU 使用率', 'machine', 'cpu', '%', 'avg', '机器非空闲 CPU 时间占比'],
  ['mem_usage_percent', '内存使用率', 'machine', 'memory', '%', 'avg', '机器物理内存使用率'],
  ['host_load_1m', '1 分钟负载', 'machine', 'load', '', 'avg', '操作系统 1 分钟平均负载'],
  ['host_load_5m', '5 分钟负载', 'machine', 'load', '', 'avg', '操作系统 5 分钟平均负载'],
  ['host_load_15m', '15 分钟负载', 'machine', 'load', '', 'avg', '操作系统 15 分钟平均负载'],
  ['host_disk_busy_percent', '磁盘 IO 忙碌率', 'machine', 'disk_io', '%', 'max', '最繁忙块设备 IO 时间占比'],
  ['host_disk_read_iops', '磁盘读 IOPS', 'machine', 'disk_io', '次/s', 'sum', '块设备每秒读操作数'],
  ['host_disk_write_iops', '磁盘写 IOPS', 'machine', 'disk_io', '次/s', 'sum', '块设备每秒写操作数'],
  ['host_disk_read_bytes_sec', '磁盘读取吞吐', 'machine', 'disk_io', 'B/s', 'sum', '块设备每秒读取字节数'],
  ['host_disk_write_bytes_sec', '磁盘写入吞吐', 'machine', 'disk_io', 'B/s', 'sum', '块设备每秒写入字节数'],
  ['host_filesystem_used_percent', '文件系统使用率', 'machine', 'filesystem', '%', 'max', '文件系统空间使用率'],
  ['host_filesystem_available_bytes', '文件系统可用空间', 'machine', 'filesystem', 'B', 'sum', '文件系统可用字节数'],
  ['host_inode_used_percent', 'Inode 使用率', 'machine', 'filesystem', '%', 'max', '文件系统 inode 使用率'],
  ['host_network_receive_bytes_sec', '网络接收吞吐', 'machine', 'network', 'B/s', 'sum', '网卡每秒接收字节数'],
  ['host_network_transmit_bytes_sec', '网络发送吞吐', 'machine', 'network', 'B/s', 'sum', '网卡每秒发送字节数'],
  ['ntp_offset_ms', '时钟偏移', 'machine', 'system', 'ms', 'max', '系统时间与 NTP 时间偏移'],
  ['agent_cpu_usage_percent', 'Agent CPU 使用率', 'machine', 'agent', '%', 'avg', 'GMHA Agent 进程 CPU 使用率'],
  ['agent_memory_rss_mb', 'Agent 内存占用', 'machine', 'agent', 'MB', 'sum', 'GMHA Agent 常驻内存']
].map(([name, display_name, scope, category, unit, aggregation, description]) => ({
  name, display_name, scope, category, unit, aggregation, description,
  value_kind: 'gauge', interval_seconds: 5, available: true
}))
...
createApp({
  ...
  setup() {
    ...
    const clusterListStats = computed(() => {
      const assignedMachines = data.value.clusters.reduce((total, item) => total + clusterMachineCount(item), 0)
      const onlineAgents = data.value.agents.filter(item => agentStatus(item).code === 'agent_online' && machineCluster(item) !== '未分配集群').length
      const mysqlInstances = data.value.mysqlInstances.filter(item => {
        const cluster = item.Cluster || item.cluster
        return cluster && cluster !== '未分配集群'
      }).length
      return {
        assignedMachines,
        onlineAgents,
        mysqlInstances,
        agentCoverage: assignedMachines ? Math.min(100, Math.round(onlineAgents * 100 / assignedMachines)) : 0
      }
    })
    ...
    const clusterOverviewInstance = ref('all')
    const clusterPerformance = ref({ summary: {}, series: [], machines: [], nodes: [], edges: [] })
    const performanceRange = ref('60')
    const performanceCustomStart = ref('')
    const performanceCustomEnd = ref('')
    const performanceLoading = ref(false)
    const performanceError = ref('')
    const performanceUpdatedAt = ref('')
    const performanceMetricRanges = ref({
      qps: '60', tps: '60', sessions: '60', locks: '60', slow: '60', replication: '60',
      cpu: '60', io: '60', disk: '60', network: '60', buffer: '60'
    })
    const performanceMetricCustomRanges = ref({})
    const performanceRangeCache = ref({})
    const performanceMetricLoading = ref({})
    const performanceTimeDialog = ref('')
    const performanceTimeForm = ref({ start: '', end: '' })
    const performanceCatalog = ref(performanceFallbackCatalog.map(item => ({ ...item })))
    const performanceScope = ref('mysql')
    const performanceCategory = ref('core')
    const performancePayloads = ref({})
    const performanceAPIUnavailable = ref(false)
    ...
    const mysqlInstallForm = ref({ machine: '', version: '', architecture: '', port: 3306, server_id: 1, mysql_user: 'mysql', root_password: '', profile: 'default', package_name: '', instance_dir: '', data_dir: '', binlog_dir: '', redo_dir: '', undo_dir: '', tmp_dir: '', base_dir: '', my_cnf_path: '', socket_path: '', error_log: '', pid_file: '', character_sets_dir: '', plugin_dir: '', install_pt_tools: false, install_xtrabackup: false, memory_allocator: 'system', runtime_parameters: mysqlRuntimeParameters, _runtime_parameter_groups: mysqlRuntimeParameterGroups, accounts: [{ role: 'monitor', username: '', password: '', host: '', enabled: true, extended_backup: false, privileges: [] }, { role: 'mha', username: '', password: '', host: '', enabled: true, extended_backup: false, privileges: [] }, { role: 'backup', username: '', password: '', host: '', enabled: true, extended_backup: false, privileges: [] }] })
	...
	const mysqlInstallXtraBackupSeries = computed(() => mysqlInstallCapabilities.value.xtraBackupSeries)
	const mysqlInstallXtraBackupPreview = computed(() => mysqlInstallCapabilities.value.xtraBackupPreview)
    const mysqlInstallUniversalGroups = computed(() => mysqlRuntimeParameterGroups.map(group => ({ ...group, fields: group.fields.map(field => {
	  const aliases = {
	  ...
	  const useAlias = (capabilities.legacy57 && ['collation_server','binlog_expire_logs_seconds'].includes(field.key)) || (capabilities.legacyTransactionVariable && field.key === 'transaction_isolation') || (capabilities.legacyReplication && ['log_slow_replica_statements','log_replica_updates'].includes(field.key)) || (capabilities.legacyRedo && field.key === 'innodb_redo_log_capacity')
	  return useAlias && aliases[field.key] ? { ...field, ...aliases[field.key] } : field
    }) })))
    const mysqlVersionParameterGroups = computed(() => selectedMySQLInstallPackage.value?.runtime_parameter_groups || [])
    const mysqlInstallParameterGroups = computed(() => [...mysqlInstallUniversalGroups.value, ...mysqlVersionParameterGroups.value])
  ...
  'PROCESS', 'RELOAD', 'LOCK TABLES', 'REPLICATION CLIENT', 'REPLICATION SLAVE', 'SUPER', 'CONNECTION_ADMIN', 'SYSTEM_VARIABLES_ADMIN', 'REPLICATION_SLAVE_ADMIN', 'REPLICATION_APPLIER', 'BACKUP_ADMIN', 'CLONE_ADMIN', 'GROUP_REPLICATION_ADMIN', 'PERSIST_RO_VARIABLES_ADMIN'
    ...
    function taskTypeLabel(value) { return ({ exec: '远程命令执行', collect_machine_info: '采集机器信息', collect_static_info: '采集静态信息', mysql_install: '安装 MySQL', mysql_uninstall: '卸载 MySQL', mysql_topology: '配置 MySQL 拓扑', mysql_upgrade: '升级 MySQL', mysql_cluster_upgrade: 'MySQL 集群滚动升级', mysql_cluster_bootstrap: '批量安装并初始化架构', batch_operation: '批量业务操作', ai_workflow: 'AI 运维工作流', architecture_adjustment: '集群架构切换', agent_recovery: 'Agent 恢复', platform_operation: '平台操作', flamegraph: '生成 Linux 火焰图' })[String(value || '').toLowerCase()] || value || '任务详情' }
    ...
    function taskTitle(item) {
      ...
      return ({
        ...
        mysql_upgrade: `升级 MySQL${port}`,
        mysql_cluster_upgrade: spec.display_name || 'MySQL 集群不停机滚动升级',
        mysql_cluster_bootstrap: spec.display_name || '批量安装并初始化架构',
        batch_operation: spec.display_name || '批量业务操作',
        ai_workflow: spec.display_name || 'AI 运维工作流',
    ...
    async function buildAgentRelease() {
      try {
        const job = await api('/upgrades/agent/build', { method: 'POST' })
        notice.value = `Agent 构建发布任务 ${job.id} 已启动，完成后自动显示新版本。`
        await loadUpgrades()
      } catch (err) { error.value = err.message }
    }
    ...
    function upgradeAgentSelectable(agent) {
      return upgradeAgentRelation(agent) === 'upgrade' && !['installing', '-', ''].includes(state(agent.install_state || agent.InstallState))
    }
    ...
    async function openClusterDetail(item) {
      ...
      clusterOverviewInstance.value = 'all'
    ...
    function clusterInstances(item) {
      const name = clusterName(item)
      if (!name) return []
      return data.value.mysqlInstances.filter(instance => String(instance.Cluster || instance.cluster || '').trim() === name)
    }
    function selectedClusterTopologyMatches(item) {
      const name = clusterName(item)
      return Boolean(name && name === clusterName(selectedClusterDetail.value) && name === String(clusterTopology.value.cluster || '').trim())
    }
    function clusterMySQLInstanceCount(item) {
      const registeredCount = clusterInstances(item).length
      if (!selectedClusterTopologyMatches(item)) return registeredCount
      return Math.max(registeredCount, (clusterTopology.value.nodes || []).length)
    }
    function clusterAbnormalInstanceCount(item) {
      const abnormal = new Set()
      clusterInstances(item).forEach(instance => {
        const status = state(instance.HeartbeatStatus || instance.heartbeat_status || instance.Status || instance.status)
        if (['fail', 'failed', 'error', 'offline'].includes(status)) {
          abnormal.add(`${instance.MachineIP || instance.machine_ip || instance.MachineID || instance.machine_id}:${Number(instance.Port || instance.port || 3306)}`)
        }
      })
      if (selectedClusterTopologyMatches(item)) {
        for (const node of (clusterTopology.value.nodes || [])) {
          const heartbeat = state(node.heartbeat || node.heartbeat_status || node.status)
          if (node.error || ['fail', 'failed', 'error', 'offline'].includes(heartbeat)) {
            abnormal.add(`${node.ip || node.machine_id}:${Number(node.port || 3306)}`)
          }
        }
      }
      return abnormal.size
    }
    ...
    async function loadClusterMachines(page = clusterMachinePage.value, clusterName = selectedClusterDetail.value?.Name || selectedClusterDetail.value?.name) {
      ...
      clusterMachineItems.value = clusterMachineItems.value.map(machine => {
        const id = machine.ID || machine.id
        const staticInfo = clusterMachineStaticInfo.value[id]
        const host = staticInfo?.Host || staticInfo?.host || staticInfo
        const architecture = host?.Arch || host?.arch || host?.Architecture || host?.architecture
        return architecture ? { ...machine, Architecture: architecture } : machine
      })
    }
    ...
    async function refreshClusterTopology(options = {}) {
      ...
      try {
        const instanceQuery = clusterOverviewInstance.value && clusterOverviewInstance.value !== 'all' ? `&instance=${encodeURIComponent(clusterOverviewInstance.value)}` : ''
        const requests = [
          api(`/clusters/${encodeURIComponent(name)}/topology?range_minutes=${clusterOverviewRange.value}${instanceQuery}`),
          api('/mysql/instances').catch(() => null)
        ]
        ...
        clusterTopologyError.value = `获取集群拓扑失败：${err.message}`
    ...
    function clusterOverview() { return clusterTopology.value.overview || { summary: {}, series: [], machines: [], storage: [] } }
    ...
    function changeClusterOverviewInstance() { refreshClusterTopology() }
    function localDateTimeValue(date) {
      const shifted = new Date(date.getTime() - date.getTimezoneOffset() * 60000)
      return shifted.toISOString().slice(0, 16)
    }
    function performanceOverview() {
      return clusterPerformance.value || { summary: {}, series: [], machines: [], nodes: [], edges: [] }
    }
    function performanceMetricName(field) {
      return ({ active_sessions: 'sessions', connected_sessions: 'sessions', running_threads: 'sessions', active_transactions: 'locks', lock_wait_sessions: 'locks', slow_queries_per_min: 'slow', replication_lag_seconds: 'replication', cpu_percent: 'cpu', io_busy_percent: 'io', disk_used_percent: 'disk', network_receive_bytes_sec: 'network', network_transmit_bytes_sec: 'network', buffer_pool_hit_ratio: 'buffer' })[field] || field
    }
    function performanceMetricRangeKey(metric) {
      const range = performanceMetricRanges.value[metric] || '60'
      if (range !== 'custom') return `preset:${range}`
      const custom = performanceMetricCustomRanges.value[metric] || {}
      return `custom:${custom.start || ''}:${custom.end || ''}`
    }
    function performanceData(metric) {
      return performanceRangeCache.value[performanceMetricRangeKey(metric)] || performanceOverview()
    }
    function performanceSummary(metric = '') { return (metric ? performanceData(metric) : performanceOverview()).summary || {} }
    function performanceSeries(metric = '') { return (metric ? performanceData(metric) : performanceOverview()).series || [] }
    function performanceTimeLabel() {
      if (performanceRange.value === 'custom') {
        if (!performanceCustomStart.value || !performanceCustomEnd.value) return '自定义时间'
        return `${new Date(performanceCustomStart.value).toLocaleString('zh-CN', { hour12: false })} 至 ${new Date(performanceCustomEnd.value).toLocaleString('zh-CN', { hour12: false })}`
      }
      return ({ '15': '近 15 分钟', '60': '近 1 小时', '360': '近 6 小时', '1440': '近 24 小时', '10080': '近 7 天' })[performanceRange.value] || '近期'
    }
    function performanceMetricRangeLabel(metric) {
      const range = performanceMetricRanges.value[metric] || '60'
      if (range === 'custom') {
        const custom = performanceMetricCustomRanges.value[metric] || {}
        if (!custom.start || !custom.end) return '自定义'
        return `${new Date(custom.start).toLocaleString('zh-CN', { hour12: false })} 至 ${new Date(custom.end).toLocaleString('zh-CN', { hour12: false })}`
      }
      return ({ '15': '15 分钟', '60': '1 小时', '360': '6 小时', '1440': '24 小时', '10080': '7 天' })[range] || '1 小时'
    }
    function performanceMetricLabel(metric) {
      return ({ qps: 'QPS', tps: 'TPS', sessions: '会话与线程', locks: '锁与事务', slow: '慢查询', replication: '复制延迟', cpu: 'CPU 使用率', io: '磁盘 IO', disk: '数据盘使用率', network: '网络吞吐', buffer: 'Buffer Pool 命中率' })[metric] || metric
    }
    function performanceChartPoints(field, width = 520, height = 150, metric = performanceMetricName(field)) {
      const values = performanceSeries(metric).map(item => Number(item[field] || 0))
      if (!values.length) return ''
      const max = Math.max(...values, 1); const min = Math.min(...values, 0); const span = Math.max(max - min, 1)
      return values.map((value, index) => `${values.length === 1 ? width / 2 : index * width / (values.length - 1)},${height - 9 - ((value - min) / span) * (height - 23)}`).join(' ')
    }
    function performanceMetricStats(field, metric = performanceMetricName(field)) {
      const values = performanceSeries(metric).map(item => Number(item[field] || 0)).filter(Number.isFinite)
      const current = Number(performanceSummary(metric)[field] || values.at(-1) || 0)
      return {
        max: values.length ? Math.max(...values) : current,
        avg: values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : current,
        current
      }
    }
    function performanceSeverity(kind) {
      const summary = performanceSummary(({ connections: 'sessions', locks: 'locks', transactions: 'locks', buffer: 'buffer', replication: 'replication' })[kind] || '')
      if (kind === 'connections') return Number(summary.connection_usage_percent || 0) >= 80 ? 'warning' : 'healthy'
      if (kind === 'locks') return Number(summary.lock_wait_sessions || 0) + Number(summary.metadata_lock_waits || 0) > 0 ? 'critical' : 'healthy'
      if (kind === 'transactions') return Number(summary.longest_transaction_seconds || 0) >= 300 ? 'warning' : 'healthy'
      if (kind === 'buffer') return Number(summary.buffer_pool_hit_ratio || 0) > 0 && Number(summary.buffer_pool_hit_ratio || 0) < 99 ? 'warning' : 'healthy'
      if (kind === 'replication') return Number(summary.replication_lag_seconds || 0) >= 30 ? 'critical' : 'healthy'
      return 'healthy'
    }
    function performanceDiagnostic(kind) {
      const summary = performanceSummary(({ connections: 'sessions', locks: 'locks', transactions: 'locks', buffer: 'buffer', replication: 'replication' })[kind] || '')
      if (kind === 'connections') return Number(summary.connection_usage_percent || 0) >= 80 ? '连接使用率偏高，建议检查连接池上限与空闲会话。' : `连接使用率 ${overviewNumber(summary.connection_usage_percent)}%，当前容量充足。`
      if (kind === 'locks') return Number(summary.lock_wait_sessions || 0) + Number(summary.metadata_lock_waits || 0) > 0 ? `发现 ${overviewNumber(Number(summary.lock_wait_sessions || 0) + Number(summary.metadata_lock_waits || 0))} 个锁等待，请检查阻塞事务。` : '未发现行锁或元数据锁等待。'
      if (kind === 'transactions') return Number(summary.longest_transaction_seconds || 0) >= 300 ? `最长事务已运行 ${overviewNumber(summary.longest_transaction_seconds)} 秒。` : '事务运行时长处于正常范围。'
      if (kind === 'buffer') return Number(summary.buffer_pool_hit_ratio || 0) > 0 ? `Buffer Pool 命中率 ${overviewNumber(summary.buffer_pool_hit_ratio, 2)}%。` : '正在等待 Buffer Pool 命中率数据。'
      return Number(summary.replication_lag_seconds || 0) >= 30 ? `复制延迟达到 ${overviewNumber(summary.replication_lag_seconds)} 秒。` : '复制链路延迟正常。'
    }
    const corePerformanceMetrics = [
      'mysql_qps', 'mysql_tps', 'mysql_threads_connected', 'mysql_threads_running',
      'mysql_active_connections', 'mysql_lock_wait_sessions', 'mysql_blocked_sessions',
      'mysql_metadata_lock_waits', 'mysql_active_transactions',
      'mysql_longest_transaction_seconds', 'mysql_slow_queries_per_min',
      'mysql_replication_lag', 'mysql_buffer_pool_hit_ratio'
    ]
    function performanceCategoryLabel(value) {
      return ({ core: '核心指标', connection: '连接与会话', performance: '吞吐与执行', replication: '复制', storage: '存储与缓存', topology: '拓扑', variables: '配置状态', cpu: 'CPU', memory: '内存', load: '系统负载', disk_io: '磁盘 IO', filesystem: '文件系统', network: '网络', system: '系统', agent: 'Agent' })[value] || value
    }
    function performanceCategories() {
      const values = [...new Set(performanceCatalog.value.filter(item => item.scope === performanceScope.value).map(item => item.category))]
      if (performanceScope.value === 'mysql') values.unshift('core')
      return values
    }
    function performanceVisibleMetrics() {
      let items = performanceCatalog.value.filter(item => item.scope === performanceScope.value)
      if (performanceCategory.value === 'core') items = items.filter(item => corePerformanceMetrics.includes(item.name))
      else items = items.filter(item => item.category === performanceCategory.value)
      return items
    }
    function performanceAPIData(metric) {
      return performancePayloads.value[performanceAPIRangeKey(metric)] || emptyPerformancePayload(metric)
    }
    function emptyPerformancePayload(metric) {
      return {
        metric: performanceCatalog.value.find(item => item.name === metric) || null,
        statistics: { current: null, min: null, max: null, average: null, p95: null },
        freshness: { last_collected_at: null, age_seconds: null, stale: true, successful_samples: 0, failed_samples: 0 },
        series: [], latest_values: [], data_points: 0
      }
    }
    function performanceAPIRangeKey(metric) {
      const range = performanceMetricRanges.value[metric] || '60'
      if (range !== 'custom') return `${metric}:preset:${range}`
      const custom = performanceMetricCustomRanges.value[metric] || {}
      return `${metric}:custom:${custom.start || ''}:${custom.end || ''}`
    }
    function performanceFormatValue(value, unit = '') {
      if (value === null || value === undefined || !Number.isFinite(Number(value))) return '—'
      const number = Number(value)
      if (unit === 'B' || unit === 'B/s') return overviewBytes(number, unit === 'B/s')
      const formatted = Math.abs(number) >= 1000
        ? number.toLocaleString('zh-CN', { maximumFractionDigits: 1 })
        : number.toLocaleString('zh-CN', { maximumFractionDigits: Math.abs(number) < 10 ? 2 : 1 })
      return `${formatted}${unit ? ` ${unit}` : ''}`
    }
    function performanceRawValue(metric) {
      const item = performanceAPIData(metric).latest_values?.[0]
      if (!item) return '—'
      const value = item.value
      if (typeof value === 'boolean') return value ? '正常 / 开启' : '异常 / 关闭'
    ...
    function performanceChartModel(metric) {
      const payload = performanceAPIData(metric)
      const series = payload.series || []
      const width = 760; const height = 270
      const left = 72; const right = 18; const top = 18; const bottom = 38
      ...
      if (maximum === minimum) maximum = minimum + Math.max(Math.abs(minimum) * .1, 1)
      const padding = (maximum - minimum) * .08
      maximum += padding
      const span = maximum - minimum
      const points = series.map((item, index) => ({
        x: series.length === 1 ? left + plotWidth / 2 : left + index * plotWidth / (series.length - 1),
        y: top + (maximum - Number(item.value)) / span * plotHeight,
        value: Number(item.value), timestamp: item.timestamp
      }))
      const yTicks = Array.from({ length: 5 }, (_, index) => {
        const value = maximum - index * span / 4
        return { y: top + index * plotHeight / 4, value, label: performanceAxisValue(value, payload.metric?.unit) }
      })
      const indexes = series.length ? [...new Set([0, Math.floor((series.length - 1) / 2), series.length - 1])] : []
      const xTicks = indexes.map(index => ({ x: points[index].x, label: new Date(series[index].timestamp).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }) }))
      return { width, height, left, right, top, bottom, plotWidth, plotHeight, points, polyline: points.map(item => `${item.x},${item.y}`).join(' '), yTicks, xTicks }
    }
    function performanceAxisValue(value, unit = '') {
      if (unit === 'B' || unit === 'B/s') {
        const text = overviewBytes(Math.max(0, value), unit === 'B/s')
        return text.replace('/s', '')
      }
      const absolute = Math.abs(value)
      if (absolute >= 1000000) return `${(value / 1000000).toFixed(1)}M`
      if (absolute >= 1000) return `${(value / 1000).toFixed(1)}k`
      return value.toLocaleString('zh-CN', { maximumFractionDigits: absolute < 10 ? 2 : 1 })
    }
    async function loadPerformanceCatalog() {
      try {
        const payload = await api('/performance/catalog')
        if (!Array.isArray(payload.items) || !payload.items.length) {
          performanceAPIUnavailable.value = true
          return
        }
        performanceCatalog.value = payload.items
        performanceAPIUnavailable.value = false
      } catch (err) {
        if (err.status === 404) {
          performanceAPIUnavailable.value = true
          return
    ...
    async function loadPerformanceAPIMetric(metric) {
      const cluster = selectedClusterDetail.value?.Name || selectedClusterDetail.value?.name
      if (!cluster || performanceMetricLoading.value[metric]) return
      if (performanceAPIUnavailable.value) {
        performancePayloads.value = { ...performancePayloads.value, [performanceAPIRangeKey(metric)]: emptyPerformancePayload(metric) }
        return
      }
      const range = performanceMetricRanges.value[metric] || '60'
      const query = new URLSearchParams({ cluster, metric })
      if (range === 'custom') {
        const custom = performanceMetricCustomRanges.value[metric] || {}
        if (!custom.start || !custom.end) return
        query.set('start_at', new Date(custom.start).toISOString())
        query.set('end_at', new Date(custom.end).toISOString())
      } else query.set('range_minutes', range)
      performanceMetricLoading.value = { ...performanceMetricLoading.value, [metric]: true }
      try {
        const payload = await api(`/performance/metrics?${query}`)
        performancePayloads.value = { ...performancePayloads.value, [performanceAPIRangeKey(metric)]: payload }
        performanceUpdatedAt.value = new Date().toISOString()
      } catch (err) {
        performancePayloads.value = { ...performancePayloads.value, [performanceAPIRangeKey(metric)]: emptyPerformancePayload(metric) }
        if (err.status === 404) performanceAPIUnavailable.value = true
        else performanceError.value = err.message
      } finally {
        performanceMetricLoading.value = { ...performanceMetricLoading.value, [metric]: false }
      }
    }
    async function loadVisiblePerformanceMetrics() {
      performanceLoading.value = true
      performanceError.value = ''
      try {
        await loadPerformanceCatalog()
        await Promise.all(performanceVisibleMetrics().filter(item => item.available).map(item => loadPerformanceAPIMetric(item.name)))
      } catch (err) {
        performanceError.value = err.message
      } finally {
        performanceLoading.value = false
      }
    }
    async function choosePerformanceScope(scope) {
      performanceScope.value = scope
      if (scope === 'flamegraph' || scope === 'memory') {
        stopPerformanceAutoRefresh()
        return
      }
      performanceCategory.value = scope === 'mysql' ? 'core' : (performanceCatalog.value.find(item => item.scope === scope)?.category || '')
      await loadVisiblePerformanceMetrics()
    }
    async function choosePerformanceCategory(category) {
      performanceCategory.value = category
      await loadVisiblePerformanceMetrics()
    }
    async function loadClusterPerformance(options = {}) {
      const name = selectedClusterDetail.value?.Name || selectedClusterDetail.value?.name
      if (!name || performanceLoading.value) return
      if (performanceRange.value === 'custom' && (!performanceCustomStart.value || !performanceCustomEnd.value)) {
        performanceError.value = '请选择完整的开始时间和结束时间。'
        return
      }
      performanceLoading.value = true
      performanceError.value = ''
      try {
        ...
        if (performanceRange.value === 'custom') {
          const start = new Date(performanceCustomStart.value)
          const end = new Date(performanceCustomEnd.value)
          if (!Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime()) || start >= end) throw new Error('开始时间必须早于结束时间。')
          if (end.getTime() - start.getTime() > 7 * 24 * 60 * 60 * 1000) throw new Error('当前性能快照保留 7 天，单次分析的时间跨度不能超过 7 天。')
          query.set('start_at', start.toISOString())
          query.set('end_at', end.toISOString())
        } else query.set('range_minutes', performanceRange.value)
        const topology = await api(`/clusters/${encodeURIComponent(name)}/topology?${query}`)
        clusterPerformance.value = { ...(topology.overview || {}), nodes: topology.nodes || [], edges: topology.edges || [] }
        const cacheKey = performanceRange.value === 'custom'
          ? `custom:${performanceCustomStart.value}:${performanceCustomEnd.value}`
          : `preset:${performanceRange.value}`
        performanceRangeCache.value = { ...performanceRangeCache.value, [cacheKey]: clusterPerformance.value }
        performanceUpdatedAt.value = new Date().toISOString()
      } catch (err) {
        performanceError.value = err.message
      } finally { performanceLoading.value = false }
      if (!options.silent && performanceRange.value !== 'custom') startPerformanceAutoRefresh()
    }
    ...
    function startPerformanceAutoRefresh() {
      stopPerformanceAutoRefresh()
      if (performanceRange.value === 'custom') return
      performanceRefreshTimer = setInterval(() => {
        if (active.value === 'clusters' && selectedClusterDetail.value && data.value.clusterSection === 'performance') loadVisiblePerformanceMetrics()
      }, 10000)
    }
    async function openPerformanceMonitoring() {
      stopClusterTopologyAutoRefresh()
      data.value.clusterSection = 'performance'
      if (!performanceCustomStart.value || !performanceCustomEnd.value) {
        const end = new Date()
        performanceCustomEnd.value = localDateTimeValue(end)
        performanceCustomStart.value = localDateTimeValue(new Date(end.getTime() - 60 * 60 * 1000))
      }
      await loadVisiblePerformanceMetrics()
    }
    async function selectPerformanceRange(value) {
      performanceRange.value = String(value)
      if (performanceRange.value === 'custom') {
        stopPerformanceAutoRefresh()
        return
      }
      Object.keys(performanceMetricRanges.value).forEach(metric => { performanceMetricRanges.value[metric] = performanceRange.value })
      await loadClusterPerformance()
    }
    async function applyPerformanceCustomRange() {
      performanceRange.value = 'custom'
      stopPerformanceAutoRefresh()
      await loadClusterPerformance()
      if (!performanceError.value) {
        Object.keys(performanceMetricRanges.value).forEach(metric => {
          performanceMetricRanges.value[metric] = 'custom'
          performanceMetricCustomRanges.value[metric] = { start: performanceCustomStart.value, end: performanceCustomEnd.value }
        })
        performanceMetricCustomRanges.value = { ...performanceMetricCustomRanges.value }
      }
    }
    async function loadPerformanceMetric(metric) {
      const name = selectedClusterDetail.value?.Name || selectedClusterDetail.value?.name
      if (!name || performanceMetricLoading.value[metric]) return
      const range = performanceMetricRanges.value[metric] || '60'
      const query = new URLSearchParams()
      if (range === 'custom') {
        const custom = performanceMetricCustomRanges.value[metric] || {}
      ...
      try {
        const topology = await api(`/clusters/${encodeURIComponent(name)}/topology?${query}`)
        const payload = { ...(topology.overview || {}), nodes: topology.nodes || [], edges: topology.edges || [] }
        performanceRangeCache.value = { ...performanceRangeCache.value, [performanceMetricRangeKey(metric)]: payload }
        performanceUpdatedAt.value = new Date().toISOString()
      } catch (err) {
        performanceError.value = err.message
      } finally {
    ...
    async function refreshAllPerformanceMetrics(silent = false) {
      await loadClusterPerformance({ silent: true })
      const loaded = new Set([performanceRange.value === 'custom'
        ? `custom:${performanceCustomStart.value}:${performanceCustomEnd.value}`
        : `preset:${performanceRange.value}`])
      const requests = []
      for (const metric of Object.keys(performanceMetricRanges.value)) {
        const key = performanceMetricRangeKey(metric)
        if (loaded.has(key) || key.startsWith('custom:')) continue
        loaded.add(key)
        requests.push(loadPerformanceMetric(metric))
      }
      await Promise.all(requests)
      if (!silent && performanceRange.value !== 'custom') startPerformanceAutoRefresh()
    }
    async function selectPerformanceMetricRange(metric, value) {
      if (value === 'custom') {
        const existing = performanceMetricCustomRanges.value[metric]
        const end = new Date()
        performanceTimeForm.value = existing
          ? { start: existing.start, end: existing.end }
          : { start: localDateTimeValue(new Date(end.getTime() - 60 * 60 * 1000)), end: localDateTimeValue(end) }
        performanceTimeDialog.value = metric
        return
      }
      performanceMetricRanges.value[metric] = String(value)
      performanceMetricRanges.value = { ...performanceMetricRanges.value }
      await loadPerformanceAPIMetric(metric)
    }
    async function applyPerformanceMetricCustomRange() {
      const metric = performanceTimeDialog.value
      if (!metric) return
      const start = new Date(performanceTimeForm.value.start)
      const end = new Date(performanceTimeForm.value.end)
      if (!Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime()) || start >= end) {
        performanceError.value = '开始时间必须早于结束时间。'
        return
      }
      if (end.getTime() - start.getTime() > 7 * 24 * 60 * 60 * 1000) {
        performanceError.value = '当前性能快照保留 7 天，自定义跨度不能超过 7 天。'
        return
      }
      performanceMetricCustomRanges.value = { ...performanceMetricCustomRanges.value, [metric]: { start: performanceTimeForm.value.start, end: performanceTimeForm.value.end } }
      performanceMetricRanges.value = { ...performanceMetricRanges.value, [metric]: 'custom' }
      performanceTimeDialog.value = ''
      await loadPerformanceAPIMetric(metric)
    }
    ...
    async function loadClusterBackups() {
      ...
      if (!cluster) return
      try {
        const [targets, policies, runs] = await Promise.all([api(`/backup/targets?cluster=${encodeURIComponent(cluster)}`), api(`/backup/policies?cluster=${encodeURIComponent(cluster)}`), api(`/backup/runs?cluster=${encodeURIComponent(cluster)}&limit=100`)])
        backupTargets.value = targets || []; backupPolicies.value = policies || []; backupRuns.value = runs || []
    ...
    function backupMachines() { const ids=new Set(backupTargets.value.filter(target=>target.backup_ready || target.machine_id===backupPolicyForm.value.machine_id).map(target=>target.machine_id)); return clusterMachineItems.value.filter(machine=>ids.has(machine.ID||machine.id)) }
    function backupInstancesForMachine(machineID) { return backupTargets.value.filter(target=>target.machine_id===machineID) }
    ...
    async function saveBackupPolicy() {
      ...
      const path = backupPolicyForm.value.id ? `/backup/policies/${encodeURIComponent(backupPolicyForm.value.id)}` : '/backup/policies'
      try { await api(path, { method:backupPolicyForm.value.id?'PUT':'POST', body:JSON.stringify(payload) }); notice.value = 'XtraBackup 备份策略已保存，Manager 调度器会按下一次执行时间下发给 Agent。'; showBackupPolicyEditor.value=false; await loadClusterBackups() }
      catch (err) { error.value = err.message }
    }
...
if (!confirm(`确认向集群 ${cluster} 的集群内所有机器创建 MySQL 安装任务？`)) return
    ...
    function automationClusterNames() {
      return [...new Set(data.value.clusters.map(item => item.Name || item.name).filter(Boolean))]
    }
    function automationAllClustersSelected() {
      const names = automationClusterNames()
      const selected = new Set(automationSelectedClusters.value)
      return names.length > 0 && names.every(name => selected.has(name))
    }
    ...
    function automationCollectionColumns(operation) {
      ...
      return [...common.slice(0, 3), { key: 'hostname', label: 'MySQL 主机名' }, { key: 'mysql_version', label: '版本' }, { key: 'mysql_port', label: '端口' }, { key: 'qps', label: 'QPS' }, { key: 'tps', label: 'TPS' }, { key: 'threads_connected', label: '当前连接' }, { key: 'threads_running', label: '运行线程' }, { key: 'questions', label: 'Questions' }, { key: 'queries', label: 'Queries' }, { key: 'slow_queries', label: '慢查询' }, { key: 'uptime', label: '运行秒数' }, ...common.slice(3)]
    }
    function setAutomationCollection(operation, rows, ready) {
      const columns = automationCollectionColumns(operation)
      const escapeCSV = value => `"${String(value ?? '').replaceAll('"', '""')}"`
      const csv = [columns.map(column => escapeCSV(column.label)).join(','), ...rows.map(row => columns.map(column => escapeCSV(column.key === 'ntp_enabled' ? (row.status === 'success' ? (row[column.key] ? '已启用' : '未启用') : '') : row[column.key])).join(','))].join('\r\n')
      data.value.automationCollection = {
        operation, rows, columns, ready,
        csv_url: rows.length ? `data:text/csv;charset=utf-8,${encodeURIComponent('\ufeff' + csv)}` : '',
        file_name: `gmha-${operation}-${new Date().toISOString().replaceAll(':', '-').slice(0, 19)}.csv`
      }
    }
    async function waitForAutomationCollection(operation, items) {
      const taskItems = items.filter(item => item.task_id)
      const clusterByTask = Object.fromEntries(taskItems.map(item => [item.task_id, item.cluster]))
      const submissionFailures = items.filter(item => !item.task_id).map(item => ({ cluster: item.cluster, machine: item.machine, ip: item.ip, status: 'failed', error: item.error || '未能启动采集' }))
      setAutomationCollection(operation, submissionFailures, taskItems.length === 0)
      if (!taskItems.length) return data.value.automationCollection.rows
      const taskIDs = taskItems.map(item => item.task_id)
      for (let attempt = 0; attempt < 120; attempt++) {
        const batches = []
        for (let offset = 0; offset < taskIDs.length; offset += 100) batches.push(taskIDs.slice(offset, offset + 100))
        const responses = await Promise.all(batches.map(ids => api(`/tasks/cluster-automation/results?operation=${encodeURIComponent(operation)}&task_ids=${encodeURIComponent(ids.join(','))}`)))
        const rows = responses.flatMap(response => response.rows || []).map(row => ({ ...row, cluster: clusterByTask[row.task_id] || '' }))
        const ready = responses.every(response => response.ready)
        setAutomationCollection(operation, [...rows, ...submissionFailures], ready)
        if (ready) return data.value.automationCollection.rows
        await new Promise(resolve => setTimeout(resolve, 1000))
      }
      const rows = data.value.automationCollection.rows.map(row => ['pending','sent','running'].includes(String(row.status).toLowerCase()) ? { ...row, status: 'failed', error: '等待采集结果超时' } : row)
      setAutomationCollection(operation, rows, true)
      return rows
    }
    async function waitForAutomationInspection(operation, items) {
      const taskItems = items.filter(item => item.task_id)
      const taskIDs = taskItems.map(item => item.task_id)
      const clusterByTask = Object.fromEntries(taskItems.map(item => [item.task_id, item.cluster]))
      const failures = items.filter(item => !item.task_id).map(item => ({ cluster: item.cluster, machine: item.machine, ip: item.ip, status: 'failed', error: item.error || '未能启动巡检', score: 0 }))
      data.value.automationInspection = { operation, ready: taskIDs.length === 0, task_ids: taskIDs, targets: failures, checks: [] }
      if (!taskIDs.length) return data.value.automationInspection
      for (let attempt = 0; attempt < 180; attempt++) {
        const batches = []
        for (let offset = 0; offset < taskIDs.length; offset += 100) batches.push(taskIDs.slice(offset, offset + 100))
        const responses = await Promise.all(batches.map(ids => api(`/tasks/database-inspection/results?task_ids=${encodeURIComponent(ids.join(','))}`)))
        const targets = responses.flatMap(response => response.targets || []).map(target => ({ ...target, cluster: target.cluster || clusterByTask[target.task_id] || '' }))
        const checks = responses.flatMap(response => response.checks || []).map(check => ({ ...check, cluster: check.cluster || clusterByTask[check.task_id] || '' }))
        const ready = responses.every(response => response.ready)
        data.value.automationInspection = { operation, ready, task_ids: taskIDs, targets: [...targets, ...failures], checks }
        if (ready) return data.value.automationInspection
        await new Promise(resolve => setTimeout(resolve, 1000))
      }
      data.value.automationInspection.ready = true
      data.value.automationInspection.targets = data.value.automationInspection.targets.map(target => ['pending','sent','running'].includes(String(target.status).toLowerCase()) ? { ...target, status: 'failed', error: '等待巡检结果超时' } : target)
      return data.value.automationInspection
    }
    async function submitAutomationTask() {
      const available = new Set(automationClusterNames())
      const clusters = [...new Set(automationSelectedClusters.value)].filter(cluster => available.has(cluster))
      automationSelectedClusters.value = clusters
      ...
      data.value.automationInspection = { operation: '', ready: false, task_ids: [], targets: [], checks: [] }
      error.value = ''
      try {
        if (form.action === 'mysql_cluster_upgrade') {
          ...
          const startResults = await Promise.allSettled(clusters.map(cluster => api('/tasks/mysql-cluster-upgrade/start', {
            method: 'POST',
            body: JSON.stringify({ cluster, target_version: form.target_version, port: Number(form.port), risk_acknowledged: true })
          })))
          automationResults.value = startResults.map((result, index) => result.status === 'fulfilled'
            ? { cluster: clusters[index], machine: '集群滚动升级', task_id: result.value.run_id, status: 'success', message: `已启动 MySQL ${form.target_version} 不停机滚动升级` }
            : { cluster: clusters[index], machine: '集群滚动升级', status: 'failed', message: result.reason?.message || '启动失败' })
        ...
        if (['collect_machine','collect_mysql'].includes(form.action)) {
          const result = await api('/tasks/cluster-automation', { method: 'POST', body: JSON.stringify({ ...form, clusters, operation: form.action }) })
          const rows = await waitForAutomationCollection(form.action, result.items || [])
          const succeeded = rows.filter(row => row.status === 'success').length
          notice.value = `采集完成：${succeeded}/${rows.length} 台机器成功，结果已显示在当前页面。`
          if (succeeded !== rows.length) error.value = '部分目标采集失败，错误原因已显示在结果表格中。'
          await refresh()
          return
        }
        if (['database_inspection','database_deep_inspection'].includes(form.action)) {
          const result = await api('/tasks/cluster-automation', { method: 'POST', body: JSON.stringify({ ...form, clusters, operation: form.action }) })
          const inspection = await waitForAutomationInspection(form.action, result.items || [])
          const succeeded = inspection.targets.filter(target => target.status === 'success').length
          notice.value = `数据库巡检完成：${succeeded}/${inspection.targets.length} 个实例成功。`
          if (succeeded !== inspection.targets.length) error.value = '部分实例巡检失败，原因已显示在巡检汇总中。'
          await refresh()
          return
        }
    ...
    function clusterAgentCount(item) {
      const name = item?.Name || item?.name
      return data.value.agents.filter(agent => machineCluster(agent) === name && agentStatus(agent).code === 'agent_online').length
    }
    function clusterAgentHealth(item) {
      const machines = clusterMachineCount(item)
      const online = clusterAgentCount(item)
      // Agent heartbeats remain the fallback when the detail topology snapshot is unavailable or stale.
      let countCode = 'pending'
      if (machines) countCode = online >= machines ? 'healthy' : (!online ? 'offline' : 'warning')
      if (clusterName(item) === clusterName(selectedClusterDetail.value)) {
        let code = selectedClusterTopologyMatches(item) && !clusterTopologyError.value ? (clusterTopology.value.agent_health || countCode) : countCode
        if (countCode === 'offline' || countCode === 'warning' || countCode === 'pending') code = countCode
        if (code === 'healthy' && clusterAbnormalInstanceCount(item) > 0) code = 'warning'
        const warningLabel = online > 0 && online < machines ? '部分离线' : '集群异常'
        return { code, label: ({ healthy: '运行正常', offline: 'Agent 离线', warning: warningLabel, pending: '待配置', unknown: '状态未知' })[code] || '状态未知' }
      }
      if (!machines) return { code: 'pending', label: '待配置' }
      if (online >= machines) return { code: 'healthy', label: '运行正常' }
      if (!online) return { code: 'offline', label: 'Agent 离线' }
      return { code: 'warning', label: '部分离线' }
    }
    ...
    function errorSummary(message, limit = 180) {
	  ...
	  if (/i\/o timeout|connection timed out|deadline exceeded/i.test(normalized) && /(?:dial tcp\s+)?[0-9a-f:.]+:\d+/i.test(normalized)) return `连接 ${endpoint} 超时：Manager 无法到达目标端口。若 Agent 在线，请更新 Manager 后重试在线推送；否则请检查目标机 SSH、路由、防火墙和安全组。`
    ...
    <main :class="['shell', { 'cluster-focus-mode': !!selectedClusterDetail }]">
      ...
      <aside class="sidebar">
        <nav v-for="group in visibleNavGroups" :key="group.title" class="nav-group" :class="{ open: expandedNav[group.title], 'cluster-nav-group': group.title === '集群运维' }">
          ...
          <div v-show="expandedNav[group.title]" class="nav-children">
            <button v-for="item in group.items" :key="item.id" class="nav-item" :aria-label="item.label" :class="{ active: active === item.id }" @click="chooseNavigation(item.id)">
      ...
      <section class="content">
        <header class="topbar"><div><h1>{{ current.label }}</h1><span class="title-refresh" :class="{ spinning: loading }" @click="refresh" title="刷新当前数据">↻</span></div>
          <div class="top-actions"><span class="updated" v-if="loading">正在刷新…</span><button v-if="active === 'machines'" class="secondary top-secondary" @click="showCredential = true">＋ 添加凭证</button><button v-if="active === 'machines'" class="primary" @click="showOnboard = true">＋ 纳管机器</button></div>
        </header>
        ...
        <ManagerConsole v-if="active === 'manager'" :machines="data.machines" :upgrade="upgradeOverview" @refresh="refresh" />
            ...
            <form class="automation-workspace" @submit.prevent="submitAutomationTask">
              ...
              <section class="panel automation-config">
                ...
                <div class="automation-action-picker">
                  ...
                  <label :class="{selected:automationForm.action==='database_inspection'}"><input v-model="automationForm.action" type="radio" value="database_inspection"><span><b>数据库巡检</b><small>批量检查连接、负载、慢查询、日志和持久性</small></span></label>
                  <label :class="{selected:automationForm.action==='database_deep_inspection'}"><input v-model="automationForm.action" type="radio" value="database_deep_inspection"><span><b>数据库深度巡检</b><small>增加表结构、碎片、长事务与 InnoDB 深层检查</small></span></label>
                  <label :class="{selected:automationForm.action==='mysql_cluster_upgrade'}"><input v-model="automationForm.action" type="radio" value="mysql_cluster_upgrade"><span><b>MySQL 集群滚动升级</b><small>从库逐台升级，安全切主后升级原主并切回</small></span></label>
...
<button :class="{active:taskFilter==='skipped'}" @click="taskFilter='skipped'">已跳过</button>
      ...
      <section v-if="active === 'clusters'" :class="['cluster-dashboard', { 'cluster-overview-mode': data.clusterSection==='overview', 'cluster-machine-mode': data.clusterSection==='machines', 'cluster-instance-mode': data.clusterSection==='instances', 'cluster-performance-mode': data.clusterSection==='performance', 'cluster-sql-diagnostic-mode': data.clusterSection==='sql-diagnostics' }]">
        <aside v-if="selectedClusterDetail" class="cluster-context-nav">
          <div class="context-cluster"><b>{{ selectedClusterDetail.Name || selectedClusterDetail.name }}</b><small>集群 ID：{{ selectedClusterDetail.ID || selectedClusterDetail.id || '—' }}</small><span :class="['cluster-health', clusterAgentHealth(selectedClusterDetail).code]"><i></i>{{ clusterAgentHealth(selectedClusterDetail).label }}</span></div>
          <button :class="{active: (data.clusterSection || 'overview')==='overview'}" type="button" @click="stopPerformanceAutoRefresh(); stopClusterTopologyAutoRefresh(); data.clusterSection='overview'; refresh()">概览</button>
          <button :class="{active: data.clusterSection==='performance'}" type="button" @click="openPerformanceMonitoring">性能监控</button>
        ...
        <template v-if="selectedClusterDetail">
          ...
          <div class="cluster-summary cluster-detail-summary"><article><small>集群机器</small><b>{{ clusterMachineCount(selectedClusterDetail) }}</b></article><article><small>MySQL 实例</small><b>{{ clusterMySQLInstanceCount(selectedClusterDetail) }}</b></article><article><small>异常实例</small><b>{{ clusterAbnormalInstanceCount(selectedClusterDetail) }}</b></article><article><small>创建时间</small><strong>{{ date(selectedClusterDetail.CreatedAt || selectedClusterDetail.created_at) }}</strong></article></div>
          ...
          <SQLDiagnostics v-else-if="data.clusterSection==='sql-diagnostics'" :mysql-instances="data.mysqlInstances" :clusters="data.clusters" :fixed-cluster="selectedClusterDetail.Name || selectedClusterDetail.name" />
          <section v-else-if="data.clusterSection==='performance'" class="performance-monitoring-workspace performance-v2">
            <header class="performance-page-head">
              <div><p>PERFORMANCE OBSERVABILITY</p><h3>性能监控</h3><span>{{ performanceScope==='flamegraph' ? '采集 Linux 调用栈，定位 CPU 热点、锁竞争与异常执行路径。' : performanceScope==='memory' ? '关联机器内存、MySQL 进程与数据库内部模块，识别内存压力来源。' : '每张卡片只展示一个指标；数值、坐标轴与采集新鲜度均来自持久化时序数据。' }}</span></div>
              <div v-if="!['flamegraph','memory'].includes(performanceScope)" class="performance-live"><span :class="{loading:performanceLoading,unavailable:performanceAPIUnavailable}"><i></i>{{ performanceLoading ? '正在读取' : performanceAPIUnavailable ? '等待性能数据接口' : '10 秒自动刷新' }}</span><small>更新于 {{ performanceUpdatedAt ? date(performanceUpdatedAt) : '—' }}</small><button class="icon-button" type="button" :disabled="performanceLoading" title="刷新当前分类" @click="loadVisiblePerformanceMetrics()">↻</button></div>
            </header>
            <nav class="performance-scope-tabs" aria-label="指标范围"><button type="button" :class="{active:performanceScope==='mysql'}" @click="choosePerformanceScope('mysql')">数据库指标</button><button type="button" :class="{active:performanceScope==='machine'}" @click="choosePerformanceScope('machine')">机器指标</button><button type="button" :class="{active:performanceScope==='memory'}" @click="choosePerformanceScope('memory')">内存分析</button><button type="button" :class="{active:performanceScope==='flamegraph'}" @click="choosePerformanceScope('flamegraph')">火焰图</button></nav>
            <template v-if="!['flamegraph','memory'].includes(performanceScope)">
            <nav class="performance-category-tabs" aria-label="指标分类"><button v-for="category in performanceCategories()" :key="category" type="button" :class="{active:performanceCategory===category}" @click="choosePerformanceCategory(category)">{{ performanceCategoryLabel(category) }}</button></nav>
            <div v-if="performanceError" class="performance-error" role="alert"><b>数据加载失败</b><span>{{ performanceError }}</span><button type="button" @click="loadVisiblePerformanceMetrics()">重试</button></div>
            <section class="performance-metric-cards" :aria-busy="performanceLoading">
              <article v-for="metric in performanceVisibleMetrics()" :key="metric.name" :class="['performance-metric-card',{unavailable:!metric.available}]">
                <header>
                  <div><span>{{ performanceCategoryLabel(metric.category) }}</span><h4>{{ metric.display_name }}</h4><p>{{ metric.description }}</p></div>
                  <select class="performance-metric-range" :disabled="!metric.available" :value="performanceMetricRanges[metric.name] || '60'" :aria-label="metric.display_name+'查看时间'" @change="selectPerformanceMetricRange(metric.name,$event.target.value)">
                    <option value="15">15 分钟</option><option value="60">1 小时</option><option value="360">6 小时</option><option value="1440">24 小时</option><option value="10080">7 天</option><option value="custom">自定义时间</option>
                  </select>
                </header>
                <section v-if="metric.available" class="performance-metric-summary">
                  <div class="performance-current"><small>当前值</small><strong>{{ performanceFormatValue(performanceAPIData(metric.name).statistics?.current, metric.unit) }}</strong><em :class="{stale:performanceAPIData(metric.name).freshness?.last_collected_at && performanceAPIData(metric.name).freshness?.stale}">{{ !performanceAPIData(metric.name).freshness?.last_collected_at ? '暂无数据' : performanceAPIData(metric.name).freshness?.stale ? '数据已过期' : '采集正常' }}</em></div>
                  <dl><div><dt>最大值</dt><dd>{{ performanceFormatValue(performanceAPIData(metric.name).statistics?.max, metric.unit) }}</dd></div><div><dt>平均值</dt><dd>{{ performanceFormatValue(performanceAPIData(metric.name).statistics?.average, metric.unit) }}</dd></div><div><dt>P95</dt><dd>{{ performanceFormatValue(performanceAPIData(metric.name).statistics?.p95, metric.unit) }}</dd></div><div><dt>样本点</dt><dd>{{ performanceAPIData(metric.name).data_points || 0 }}</dd></div></dl>
                </section>
                <div v-if="metric.available && performanceAPIData(metric.name).series?.length" class="performance-metric-plot">
                  <svg viewBox="0 0 760 270" role="img" :aria-label="metric.display_name+'时间序列图'">
                    <g class="performance-grid-lines"><line v-for="tick in performanceChartModel(metric.name).yTicks" :key="'grid-'+tick.y" :x1="performanceChartModel(metric.name).left" :y1="tick.y" x2="742" :y2="tick.y"/></g>
                    <g class="performance-y-axis"><text v-for="tick in performanceChartModel(metric.name).yTicks" :key="'y-'+tick.y" x="62" :y="tick.y+4">{{ tick.label }}</text></g>
                    <polyline class="performance-series-line" :points="performanceChartModel(metric.name).polyline"/>
                    <g class="performance-data-points"><circle v-for="point in performanceChartModel(metric.name).points" :key="point.timestamp" :cx="point.x" :cy="point.y" r="7"><title>{{ date(point.timestamp) }} · {{ performanceFormatValue(point.value,metric.unit) }}</title></circle></g>
                    <g class="performance-x-axis"><text v-for="tick in performanceChartModel(metric.name).xTicks" :key="'x-'+tick.x" :x="tick.x" y="258">{{ tick.label }}</text></g>
                  </svg>
                </div>
                <div v-else-if="metric.available && performanceAPIData(metric.name).latest_values?.length" class="performance-metric-state"><small>最新状态 / 结构化采集值</small><strong>{{ performanceRawValue(metric.name) }}</strong><span>该指标为状态或明细数据，不用折线高度表达。</span></div>
                <div v-else class="performance-metric-empty"><i>{{ metric.available ? '⌁' : '!' }}</i><b>{{ !metric.available ? '该指标尚未启用采集' : performanceMetricLoading[metric.name] ? '正在读取时序数据' : '当前时间段暂无数据' }}</b><small>{{ !metric.available ? (metric.unavailable_reason || '当前 Agent 版本没有对应采集器') : '请确认 Agent 在线且该指标已完成至少一次采集。' }}</small></div>
                <footer><span>单位 {{ metric.unit || '数值' }} · 聚合 {{ metric.aggregation }}</span><span v-if="metric.available">最近采集 {{ performanceAPIData(metric.name).freshness?.last_collected_at ? date(performanceAPIData(metric.name).freshness.last_collected_at) : '—' }}</span><span v-else>指标已登记 · 不隐藏缺项</span></footer>
              </article>
            </section>
            <div v-if="performanceTimeDialog" class="modal-mask performance-time-dialog-mask" @click.self="performanceTimeDialog=''">
              <form class="modal performance-time-dialog" @submit.prevent="applyPerformanceMetricCustomRange">
                <div class="modal-head"><div><p>INDIVIDUAL TIME RANGE</p><h2>{{ performanceCatalog.find(item=>item.name===performanceTimeDialog)?.display_name || performanceTimeDialog }} · 自定义时间</h2></div><button type="button" @click="performanceTimeDialog=''">×</button></div>
                <p class="form-note">该时间只影响当前指标，不改变其他卡片。性能时序数据保留 7 天。</p>
                <div class="performance-dialog-fields"><label>开始时间<input v-model="performanceTimeForm.start" type="datetime-local" required></label><label>结束时间<input v-model="performanceTimeForm.end" type="datetime-local" required></label></div>
                <div class="modal-actions"><button type="button" class="secondary" @click="performanceTimeDialog=''">取消</button><button class="primary">应用到当前指标</button></div>
              </form>
            </div>
            </template>
	...
	<section class="cluster-overview-dashboard topology-workspace" aria-label="集群运行概览">
	  ...
	  <template v-else>
	    ...
	    <div class="overview-kpi-grid">
	      ...
	      <article class="disk"><header><span>磁盘</span><i>数据目录使用率</i></header><b>{{ overviewNumber(clusterOverview().summary.disk_used_percent) }}<em>%</em></b><small>数据库 datadir 所在文件系统</small><div class="overview-meter"><i :style="{width:Math.min(Number(clusterOverview().summary.disk_used_percent||0),100)+'%'}"></i></div></article>
	    </div>
	    ...
	    <section class="overview-storage-panel">
	      <header><div><b>磁盘空间容量监控</b><small>按实际文件系统展示；数据、Binlog、Redo、备份/NAS、Swap 等用途可能位于不同磁盘</small></div><div class="overview-storage-actions"><label>查看实例<select v-model="clusterOverviewInstance" @change="changeClusterOverviewInstance"><option value="all">全部实例</option><option v-for="node in clusterTopology.nodes" :key="'storage-instance-'+node.machine_id+':'+node.port" :value="String(node.machine_id)+':'+Number(node.port)">{{ node.name || node.ip }} · {{ node.ip }}:{{ node.port }}</option></select></label><span>{{ (clusterOverview().storage || []).length }} 个存储空间</span></div></header>
	      <div v-if="(clusterOverview().storage || []).length" class="overview-storage-grid">
	        <article v-for="disk in clusterOverview().storage" :key="disk.id" :class="{warning:Number(disk.used_percent)>=80,critical:Number(disk.used_percent)>=90,unavailable:!disk.available}">
	          <header><div><b>{{ disk.machine_name || disk.ip }}</b><small>{{ disk.ip }}<template v-if="disk.ports?.length"> · 端口 {{ disk.ports.join('、') }}</template></small></div><div class="overview-storage-purposes"><span v-for="purpose in disk.purposes" :key="purpose">{{ purpose }}</span></div></header>
	          <div class="overview-storage-location"><b>{{ disk.mount || disk.paths?.[0] || '等待识别' }}</b><small>{{ disk.source || disk.fs_type || '文件系统信息待上报' }}</small></div>
	          <template v-if="disk.available">
	            <div class="overview-storage-usage"><b>{{ overviewNumber(disk.used_percent) }}%</b><span>可用 {{ overviewBytes(disk.available_bytes) }}</span></div>
	            <div class="overview-storage-meter"><i :style="{width:Math.min(Number(disk.used_percent||0),100)+'%'}"></i></div>
	            <dl><div><dt>总容量</dt><dd>{{ overviewBytes(disk.total_bytes) }}</dd></div><div><dt>已使用</dt><dd>{{ overviewBytes(disk.used_bytes) }}</dd></div><div><dt>剩余</dt><dd>{{ overviewBytes(disk.available_bytes) }}</dd></div></dl>
	          </template>
	          <div v-else class="overview-storage-waiting">{{ disk.fs_type === 'swap' ? '当前机器未启用 Swap' : '等待 Agent 探测该路径容量' }}</div>
	          <footer v-if="disk.paths?.length"><span v-for="path in disk.paths" :key="path" :title="path">{{ path }}</span></footer>
	        </article>
	      </div>
	      <div v-else class="overview-storage-empty"><b>正在等待存储容量数据</b><small>Agent 升级并完成下一次 30 秒采集后，将显示各文件系统与 Swap 容量。</small></div>
	    </section>

	    <div class="overview-secondary-grid">
	      <section class="overview-capacity-panel"><header><div><b>数据库容量与碎片</b><small>业务 Schema 的 data_length、index_length 与 data_free</small></div><span>每 5 分钟采集</span></header><div class="capacity-hero"><div class="capacity-ring" :style="{'--fragment':Math.min(Number(clusterOverview().summary.fragment_percent||0),100)+'%'}"><b>{{ overviewNumber(clusterOverview().summary.fragment_percent) }}%</b><small>碎片率</small></div><div><small>{{ clusterOverviewInstance==='all' ? '数据库总量（含副本）' : '当前实例数据库总量' }}</small><b>{{ overviewBytes(Number(clusterOverview().summary.data_bytes||0)+Number(clusterOverview().summary.index_bytes||0)) }}</b><span>{{ clusterOverviewInstance==='all' ? '物理容量按所有受管实例汇总' : '仅统计当前选择的数据库实例' }}</span></div></div><dl><div><dt>表数据</dt><dd>{{ overviewBytes(clusterOverview().summary.data_bytes) }}</dd></div><div><dt>索引数据</dt><dd>{{ overviewBytes(clusterOverview().summary.index_bytes) }}</dd></div><div><dt>可回收碎片</dt><dd>{{ overviewBytes(clusterOverview().summary.fragment_bytes) }}</dd></div></dl></section>
  ...
  <div v-else class="live-topology-canvas">
    ...
    <div class="live-edge-list"><header><b>实时复制链路</b><span>{{ clusterTopology.edges.length }} 条</span></header><div><p v-for="edge in clusterTopology.edges" :key="edge.source_ip+':'+edge.source_port+'>'+edge.target_ip+':'+edge.target_port" :class="{bad:edge.io_running!=='Yes'||edge.sql_running!=='Yes'}"><span><b>{{ edge.source_name || edge.source_ip }}</b><small>{{ edge.source_ip }}:{{ edge.source_port }}</small></span><i>→</i><span><b>{{ edge.target_name || edge.target_ip }}</b><small>{{ edge.target_ip }}:{{ edge.target_port }}</small></span><em>{{ edge.io_running || '—' }}/{{ edge.sql_running || '—' }} · 延迟 {{ topologyMetric(edge.lag,'s') }}</em></p><small v-if="!clusterTopology.edges.length">当前没有复制链路，实例按独立节点展示。</small></div></div>
          ...
          <section class="cluster-workspace member-overview cluster-machine-mysql"><div class="panel-head"><div><h3>集群机器管理</h3><p>仅管理当前集群的机器成员、连接信息和 Agent 状态；移出集群不会删除机器、Agent 或 MySQL 数据。</p></div><div class="cluster-machine-actions"><span>共 {{ clusterMachineTotal }} 台机器</span><span v-if="selectedClusterOperationMachineIDs.length">已选 {{ selectedClusterOperationMachineIDs.length }} 台</span><button v-if="clusterMachineItems.length" type="button" class="secondary" @click="toggleClusterMachinePageSelection">{{ clusterMachinePageSelected() ? '取消当前页全选' : '当前页全选' }}</button><button type="button" class="danger-button" :disabled="!selectedClusterOperationMachineIDs.length" @click="removeSelectedMachinesFromCluster">批量移出</button><button class="primary" @click="openClusterMembers(selectedClusterDetail)">＋ 添加机器</button></div></div><div class="cluster-machine-table-wrap"><table v-if="clusterMachineItems.length" class="cluster-machine-table"><thead><tr><th class="machine-check-column"><input type="checkbox" :checked="clusterMachinePageSelected()" aria-label="选择当前页全部机器" @change="toggleClusterMachinePageSelection"></th><th class="machine-column">机器</th><th class="network-column">网卡 / IP 地址</th><th class="architecture-column">架构</th><th class="ssh-column">SSH</th><th class="agent-column">Agent</th><th class="credential-column">凭证</th><th class="actions-column">操作</th></tr></thead><tbody><tr v-for="machine in clusterMachineItems" :key="machine.ID || machine.id" :class="{selected:selectedClusterOperationMachineIDs.includes(machine.ID || machine.id)}"><td class="machine-check-column"><input v-model="selectedClusterOperationMachineIDs" type="checkbox" :value="machine.ID || machine.id" :aria-label="'选择机器 '+(machine.Name || machine.name)"></td><td class="machine-column"><button class="machine-name-link" @click="showMachine(machine.ID || machine.id)" :title="machine.Name || machine.name">{{ machine.Name || machine.name }}</button><small :title="machine.ID || machine.id">{{ machine.ID || machine.id }}</small></td><td class="network-column"><div class="machine-network-list"><div v-for="item in clusterMachineInterfaces(machine)" :key="item.name + item.ip" class="machine-network-item"><span>{{ item.name }}</span><b>{{ item.ip }}</b></div></div></td><td class="architecture-column">{{ machine.Architecture || machine.architecture || machine.Arch || machine.arch || '待采集' }}</td><td class="ssh-column"><b>{{ machine.SSHUser || machine.ssh_user || 'root' }}</b><small>端口 {{ machine.SSHPort || machine.ssh_port || 22 }}</small></td><td class="agent-column"><span :class="['status', machineStatus(machine).code]">{{ machineStatus(machine).label }}</span></td><td class="credential-column">{{ data.credentials.find(item => (item.id || item.ID) === (machine.CredentialID || machine.credential_id))?.name || ((machine.CredentialID || machine.credential_id) ? '已关联' : '未分配') }}</td><td class="cluster-row-actions actions-column"><button class="text-button" @click="showMachine(machine.ID || machine.id)">详情 / 编辑</button><button class="danger-link" @click="removeMachineFromCluster(machine)">移出集群</button></td></tr></tbody></table><div v-else class="member-summary">当前集群暂无机器。点击“添加机器”从已纳管机器中选择。</div></div><div v-if="clusterMachineTotal" class="cluster-pager"><span>第 {{ clusterMachinePage }} 页 · 共 {{ clusterMachineTotal }} 台机器</span><div><button class="secondary" :disabled="clusterMachinePage===1" @click="changeClusterMachinePage(-1)">上一页</button><button class="secondary" :disabled="clusterMachinePage*50>=clusterMachineTotal" @click="changeClusterMachinePage(1)">下一页</button></div></div></section>
	        ...
	        <template v-else>
	          <div class="cluster-list-hero cluster-hero">
	            <div class="cluster-list-heading">
	              <span class="cluster-list-eyebrow"><i></i> CLUSTER OPERATIONS</span>
	              <h2>集群列表</h2>
	              <p>统一查看集群资源与 Agent 健康状态，快速进入日常运维。</p>
	            </div>
	            <div class="cluster-hero-actions">
	              <button class="primary" type="button" @click="openCreateCluster"><span>＋</span> 创建集群</button>
	              <button class="secondary" type="button" @click="active='machines'">管理机器</button>
	            </div>
	          </div>
	          <div class="cluster-summary cluster-list-summary" aria-label="集群资源概览">
	            <article class="cluster-summary-card">
	              <span class="cluster-kpi-icon cluster-kpi-cluster">◇</span>
	              <div><small>集群总数</small><b>{{ clusterTotal || data.clusters.length }}<em>个</em></b><span>逻辑运维边界</span></div>
	            </article>
	            <article class="cluster-summary-card">
	              <span class="cluster-kpi-icon cluster-kpi-machine">▣</span>
	              <div><small>已分配机器</small><b>{{ clusterListStats.assignedMachines }}<em>台</em></b><span>纳入集群管理</span></div>
	            </article>
	            <article class="cluster-summary-card">
	              <span class="cluster-kpi-icon cluster-kpi-mysql">◉</span>
	              <div><small>MySQL 实例</small><b>{{ clusterListStats.mysqlInstances }}<em>个</em></b><span>已登记实例</span></div>
	            </article>
	            <article class="cluster-summary-card cluster-agent-summary">
	              <span class="cluster-kpi-icon cluster-kpi-agent">⌁</span>
	              <div><small>在线 Agent</small><b>{{ clusterListStats.onlineAgents }}<em>个</em></b><span>覆盖率 {{ clusterListStats.agentCoverage }}%</span></div>
	              <i class="cluster-coverage" :style="{ '--cluster-coverage': clusterListStats.agentCoverage + '%' }"></i>
	            </article>
	          </div>
	          <section class="cluster-workspace cluster-list-workspace">
	            <div class="panel-head cluster-list-toolbar">
	              <div>
	                <div class="cluster-list-title"><h3>全部集群</h3><span>{{ clusterTotal }} 个</span></div>
	                <p>查看资源规模、实例数量与 Agent 运行状态</p>
	              </div>
	              <form class="cluster-search" role="search" @submit.prevent="searchClusterPage">
	                <label class="cluster-search-field">
	                  <span aria-hidden="true">⌕</span>
	                  <input v-model.trim="clusterKeyword" aria-label="搜索集群名称或描述" placeholder="搜索集群名称或描述">
	                  <button v-if="clusterKeyword" type="button" aria-label="清空搜索" @click="clusterKeyword='';searchClusterPage()">×</button>
	                </label>
	                <button class="secondary cluster-search-submit">查询</button>
	                <button type="button" class="icon-button cluster-refresh" :disabled="loading" aria-label="刷新集群列表" title="刷新集群列表" @click="refresh">↻</button>
	              </form>
	            </div>
	            <div v-if="clusterPageItems.length" class="cluster-table-wrap">
	              <table class="cluster-table cluster-list-table">
	                <thead><tr><th>集群信息</th><th>资源规模</th><th>MySQL 实例</th><th>Agent 状态</th><th>创建时间</th><th><span class="sr-only">操作</span></th></tr></thead>
	                <tbody>
	                  <tr v-for="item in clusterPageItems" :key="item.Name || item.name" class="cluster-row" tabindex="0" @click="openClusterDetail(item)" @keydown.enter="openClusterDetail(item)" @keydown.space.prevent="openClusterDetail(item)">
	                    <td data-label="集群信息">
	                      <div class="cluster-identity"><span>◇</span><div><b>{{ item.Name || item.name }}</b><small :title="item.Description || item.description || '暂无描述'">{{ item.Description || item.description || '暂无描述' }}</small></div></div>
	                    </td>
	                    <td data-label="资源规模"><div class="cluster-count"><b>{{ clusterMachineCount(item) }}</b><span>台机器</span></div></td>
	                    <td data-label="MySQL 实例"><div class="cluster-count"><b>{{ clusterMySQLInstanceCount(item) }}</b><span>个实例</span></div></td>
	                    <td data-label="Agent 状态">
	                      <div class="cluster-agent-state"><span :class="['cluster-health', clusterAgentHealth(item).code]"><i></i>{{ clusterAgentHealth(item).label }}</span><small>{{ clusterAgentCount(item) }} / {{ clusterMachineCount(item) }} 在线</small></div>
	                    </td>
	                    <td data-label="创建时间"><time>{{ date(item.CreatedAt || item.created_at) }}</time></td>
	                    <td class="cluster-manage-cell"><button class="cluster-manage-button" @click.stop="openClusterDetail(item)">进入管理 <span>→</span></button></td>
	                  </tr>
	                </tbody>
	              </table>
	            </div>
	            <div v-else class="cluster-empty cluster-list-empty"><span class="cluster-empty-icon">{{ clusterKeyword ? '⌕' : '◇' }}</span><b>{{ clusterKeyword ? '未找到匹配集群' : '尚未创建集群' }}</b><small>{{ clusterKeyword ? '没有与“' + clusterKeyword + '”匹配的结果，请尝试其他关键词。' : '创建集群后，即可集中管理机器、MySQL 实例和高可用拓扑。' }}</small><button v-if="!clusterKeyword" class="primary" @click="openCreateCluster">＋ 创建第一个集群</button><button v-else class="secondary" @click="clusterKeyword='';searchClusterPage()">清空搜索</button></div>
	            <div v-if="clusterTotal" class="cluster-pager cluster-list-pager">
	              <span>共 {{ clusterTotal }} 个集群 · 第 {{ clusterPage }} / {{ Math.max(1, Math.ceil(clusterTotal / 12)) }} 页</span>
	              <div><button class="secondary" :disabled="clusterPage===1" @click="changeClusterPage(-1)">← 上一页</button><button class="secondary" :disabled="clusterPage*12>=clusterTotal" @click="changeClusterPage(1)">下一页 →</button></div>
	            </div>
	          </section>
	        </template>
      </section>
