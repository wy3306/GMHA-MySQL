...
export const manualDeepDive = {
  ...
  'ai-automation': detail(
    ...
    [
      ...
      ['分层会话记忆', '浏览器每轮只发送新增消息和稳定 session_id。Manager 持久化目标、约束、决定、进展、待确认项及服务端校验的当前意图；模型每轮只获得最多 8 条、8000 字符且约 2400 tokens 的近期原文，以及独立预算内的结构化快照，不会重传完整历史。归档只隐藏对话，新建对话隔离上下文，恢复后可继续。'],
