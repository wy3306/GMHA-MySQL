...
export const apiEndpoints = [
  ...
  endpoint('AI 与自动化', 'GET', '/ai/memory?session_id=session-01', '查看会话记忆', { response: { session_id: 'session-01', enabled: true, summary: '为 Demo01 配置业务 VIP', open_questions: [], active_intent: { action: 'configure_cluster_vip', target_id: 'Demo01', parameters: {} }, revision: 2 }, note: '摘要仅用于理解上下文；执行关键参数由服务端校验后的 active_intent 保存。' }),
  ...
  endpoint('AI 与自动化', 'POST', '/ai/chat', '向 AI 运维助手提问', { body: { session_id: 'session-01', provider_id: 'provider-01', message: '先诊断 DB-01，再根据结果安全修复' }, response: { message: { role: 'assistant', content: '已结合平台架构生成分步方案。' }, plans: [], workflows: [] }, note: '客户端每轮只提交新增消息。Manager 注入会话滚动摘要、最近 16 条原文和服务端校验的当前意图，不要求浏览器重复发送完整历史。' }),
