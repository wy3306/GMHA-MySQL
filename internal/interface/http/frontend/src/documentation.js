...
export const apiEndpoints = [
  ...
  endpoint('任务与自动化', 'GET', '/tasks?id={task_id}', '查询任务详情', { response: { task: taskResponse, steps: [], events: [], children: [], controls: { skip: { allowed: true, reason: '任务尚未下发，可以安全标记为跳过。', requires_confirmation: true, confirmation: 'SKIP task-01HX...' }, retry: { allowed: false, reason: '仅失败且支持步骤续跑的安装任务可以继续。' }, rollback: { allowed: false, reason: '仅失败任务可以重试恢复动作。' }, rollback_class: 'automatic', rollback_summary: '任务失败时 Agent 会自动执行受保护的恢复动作。' } } }),
  endpoint('任务与自动化', 'GET', '/tasks?stats=true', '查询任务统计', { response: { all: 42, running: 2, success: 37, failed: 2, skipped: 1 } }),
  endpoint('任务与自动化', 'POST', '/tasks/control', '跳过、修复后续跑或重试恢复动作', { body: { task_id: 'task-01HX...', action: 'retry', confirmation: 'RETRY task-01HX...' }, response: { action: 'retry', task: { task: { ...taskResponse, status: 'pending', progress_percent: 80 }, steps: [], events: [], controls: {} } }, note: 'action 为 skip、retry 或 rollback。retry 仅对兼容 Agent 上的失败 MySQL 安装开放，保留成功步骤并从失败步骤继续；rollback 会创建独立恢复任务并保留原任务审计记录。' }),
  ...
  endpoint('MySQL 任务', 'GET', '/mysql/linux-compatibility?machine=machine-01', '检查目标 Linux 安装兼容性', { query: ['machine'], response: { os: 'Ubuntu 22.04', distribution: 'Ubuntu', version: '22.04', major_version: 22, architecture: 'x86_64', glibc_version: '2.35', level: 'supported', can_install: true, package_manager: 'apt', summary: 'Ubuntu 20.04/22.04/24.04 属于支持范围。' }, note: '创建 MySQL 安装任务时服务端会再次执行同一检查。level 为 supported、legacy、unsupported 或 unknown；legacy 表示可运行但不属于首选平台。' }),
  ...
  endpoint('高可用', 'POST', '/clusters/{cluster_name}/architecture/plan', '生成复制架构、MGR + Router 或 VIP 漂移预检计划', { body: { architecture: 'master_slave', current_architecture: 'dual_master', current_master_machine_id: 'machine-01', preferred_new_master_machine_id: 'machine-02', move_vip: true, nodes: [{ machine_id: 'machine-01', port: 3306, role: 'S', source_machine_id: 'machine-02' }, { machine_id: 'machine-02', port: 3306, role: 'M' }] }, response: { plan_id: 'plan-architecture-01', cluster_id: 'prod', architecture: 'master_slave', executable: true, blocking_reasons: [], steps: [] }, note: '只读预检，不执行变更。mgr_router 另需 3–9 个奇数 MySQL 8.0.17+ 成员、mgr_port、router_port、MySQL Shell/Router 可信制品，并且不能使用 VIP；Router 基础端口会连续占用 4 个端口。建组前会冻结业务并建立数据/GTID 屏障：新装空节点安全清理从节点 GTID，已有数据则必须先追平且通过 PT 一致性校验。一次性 root 密码可按节点补齐旧 MHA/MGR 管理账号权限，未提供时会在执行前严格验证现有管理账号。' }),
  ...
  endpoint('高可用', 'GET', '/clusters/{cluster_name}/mgr', '查询 MGR 成员、Quorum、Router 与 AdminAPI 状态', { response: { cluster: 'prod', architecture: 'mgr_router', available: true, healthy: true, quorum: true, member_count: 3, online_member_count: 3, primary_machine_id: 'machine-01', members: [{ machine_id: 'machine-01', state: 'ONLINE', role: 'PRIMARY', apply_queue: 0, remote_apply_queue: 0, conflicts: 0, reachable: true }], routers: { routers: {} } }, note: '读取操作会通过每个节点的在线 Agent 直接采集成员状态；有 ONLINE 成员时再读取 InnoDB Cluster 与 Router 元数据。' }),
  endpoint('高可用', 'POST', '/clusters/{cluster_name}/mgr/actions', '执行 MGR 专属管理操作', { body: { action: 'set_primary', machine_id: 'machine-02', confirmation: 'SET PRIMARY machine-02' }, response: { action: 'set_primary', success: true, message: 'MGR PRIMARY 已安全切换', task_ids: ['task-01'], status: { healthy: true, quorum: true, primary_machine_id: 'machine-02' } }, note: 'action 支持 set_primary、rejoin_member、rescan_metadata、rotate_recovery_passwords、reboot_complete_outage。每项均取得集群互斥锁并要求对应精确确认；全组停机恢复要求零 ONLINE、所有成员可达，并由 AdminAPI 完成 GTID 校验，不开放 force。' }),
  ...
  endpoint('AI 与自动化', 'GET', '/ai', '读取 AI 运维工作台', { response: { providers: [], settings: { enabled: false, always_confirm_high_risk: true }, sessions: [], messages: [], memories: [], plans: [], workflows: [], runs: [], actions: [], stats: {} }, note: '提供商凭据不会返回浏览器；高危确认策略由服务端强制开启。工作流包含子操作、依赖、检查点和父任务编号。' }),
  ...
  endpoint('AI 与自动化', 'GET', '/ai/sessions?include_archived=true', '查询 AI 对话', { response: [{ id: 'session-01', title: 'Demo01 VIP 变更', status: 'active', message_count: 4 }], note: '默认只返回活动对话；include_archived=true 同时返回归档记录。归档不删除消息、计划、记忆或审计。' }),
  endpoint('AI 与自动化', 'POST', '/ai/sessions', '新建独立 AI 对话', { body: { title: 'Demo01 VIP 变更' }, response: { id: 'session-01', title: 'Demo01 VIP 变更', status: 'active' } }),
  endpoint('AI 与自动化', 'POST', '/ai/sessions/archive', '归档 AI 对话', { body: { id: 'session-01' }, response: { id: 'session-01', status: 'archived' }, note: '归档是可恢复的界面清理，不清除审计数据。' }),
  endpoint('AI 与自动化', 'POST', '/ai/sessions/restore', '恢复归档对话', { body: { id: 'session-01' }, response: { id: 'session-01', status: 'active' } }),
  endpoint('AI 与自动化', 'GET', '/ai/memory?session_id=session-01', '查看会话记忆', { response: { session_id: 'session-01', enabled: true, summary: '为 Demo01 配置业务 VIP', goals: ['配置业务 VIP'], constraints: ['不中断现有写流量'], decisions: ['使用 192.168.31.222/24'], progress: ['等待审批'], open_questions: [], active_intent: { action: 'configure_cluster_vip', target_id: 'Demo01', parameters: {} }, summarized_message_count: 24, compacted_message_count: 16, recent_message_count: 8, estimated_input_tokens: 5200, revision: 2 }, note: '完整历史保留在 Manager；模型只获得预算内的结构化快照与近期窗口。执行关键参数由服务端校验后的 active_intent 保存。' }),
  endpoint('AI 与自动化', 'PUT', '/ai/memory', '维护会话长期说明', { body: { session_id: 'session-01', enabled: true, instructions: '高风险动作先展示影响范围' }, response: { session_id: 'session-01', enabled: true }, note: '密码、Token、API Key 和私钥不得保存到会话记忆。' }),
  endpoint('AI 与自动化', 'DELETE', '/ai/memory?session_id=session-01', '清除会话记忆', { response: { deleted: true }, note: '只清除摘要和说明，不删除原始消息、计划和审计。' }),
  endpoint('AI 与自动化', 'POST', '/ai/chat', '向 AI 运维助手提问', { body: { session_id: 'session-01', provider_id: 'provider-01', message: '先诊断 DB-01，再根据结果安全修复' }, response: { message: { role: 'assistant', content: '已结合平台架构生成分步方案。' }, plans: [], workflows: [] }, note: '客户端每轮只提交新增消息。Manager 注入结构化滚动快照、最多 8 条且约 2400 tokens 的近期原文和服务端校验的当前意图；完整历史不会重传。单次新增输入也有独立上限。' }),
  ...
  endpoint('Manager 与升级', 'GET', '/upgrades/overview', '查询升级概览', { response: { manager_version: 'V1.2.0', manager_latest_version: 'V1.3.0', agent_running_latest_version: 'V1.2.1', agent_latest_version: 'V1.3.0', agent_total: 3, agent_versions: [], manager_packages: [], agent_packages: [], storage: {} } }),
