...
var mysqlKeyTextColumn = regexp.MustCompile(`(?i)\b(id|role|name|cluster_id|failover_id|run_id|machine_id|agent_id|fingerprint|kind|ip|parent_task_id|visibility|task_id|created_at|current_state|collected_at|status|metric|last_seen_at|delivered_at|next_run_at|cluster_name|vip_address|interface_name)\s+text\b`)
