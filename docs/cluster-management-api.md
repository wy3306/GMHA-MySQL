...
- `recent messages`：每轮最多加载同一会话最近 8 条、8000 个字符且约 2400 tokens 的用户/助手原文，不会混入其他会话；超长单条消息只保留有界的开头和结尾。
- `structured rolling summary`：模型每轮返回目标、约束、决定、进度和待确认项的完整当前快照，Manager 归一化、去重并按会话保存；较早原文由本地分层压缩后不再重复发送。
- `memory budget`：发送给模型的记忆快照还有独立的约 2400-token 上限。完整结构化记忆和原始消息仍保留在 Manager 审计库，模型只看到预算内的必要部分。
- `input guard`：单次新增输入最多 12000 个字符且约 4000 tokens，防止一条异常输入绕过历史窗口预算。
- `validated active intent`：当前动作、目标和参数来自服务端已校验计划，独立于模型摘要保存。VIP 地址等执行关键值不会只凭模型记忆进入执行。
- `instructions`：用户可为会话保存长期说明；密码、Bearer Token、API Key、私钥和 URL 凭据会被拒绝或在旧历史重传前脱敏。
