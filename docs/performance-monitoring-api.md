...
GMHA Manager 将 Agent 上报的数据库和机器指标保存为规范化时序样本。正常心跳历史默认保留 7 天，由独立维护任务每 2 小时分批清理；Agent 状态异常、健康检查非 OK、或指标采集失败的心跳会保留用于故障追溯。清理不在心跳写入请求中执行。所有查询均以持久化样本为准，不使用前端估算值。
