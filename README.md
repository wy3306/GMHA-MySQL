...
- Agent 增加启动、心跳和任务通道独立重连；Manager 或网络短暂不可达时会持续恢复连接，不中断已执行任务。
- Agent 新增受控文件日志：启动早期即可记录诊断，单文件 10 MiB 自动轮转、gzip 压缩、最多保留 5 份且最长 7 天。
- 新增 Agent 版本构建与发布流程：可从 Manager 控制台或命令行生成 AMD64/ARM64 同版本制品，自动登记版本、架构与 SHA-256。
- 在线 Agent 升级优先通过任务通道自下载、校验并以 systemd 临时单元替换，不依赖 Manager 入站 SSH；离线时仍安全回退到 SSH。
- Manager 控制台加强 Agent/集群健康状态、升级错误说明、制品可用性和运行时版本展示。
- 官网升级为产品实景导览，增加核心场景轮播、24 个 4K 控制台截图、完整产品导览页和缓存策略。

Bug 修复：

- 修复 Agent 仅依赖单次启动连接，Manager 暂停、网络抖动或 WebSocket 断开后可能长期离线的问题。
- 修复在线升级在 SSH 端口不可达时无法推送、替换前未严格校验候选二进制版本的问题。
- 修复 Agent 启动失败、重启限流和恢复超时时缺少 journal/文件日志上下文的问题。
- 修复集群详情健康状态可能受分页或局部筛选影响，以及升级页无法清晰区分制品、架构和失败原因的问题。
...
- 新增 MGR 专属管理与 MySQL Router 路由能力，支持 MGR 状态、法定人数、成员角色、AdminAPI 与 Router 元数据检查，以及受控的成员维护操作。
- 新增任务中心统一口径和服务端控制能力，支持待执行任务跳过、失败 MySQL 安装修复后续跑、独立恢复重试，并展示父任务、执行节点、步骤和事件的聚合状态。
- 集群滚动升级扩展至 MGR 架构，增加成员状态、角色、Group Replication 信息和升级阶段预检。
- 新增 Linux 兼容矩阵与 GitHub Actions 兼容性流水线；Agent 同时提供 linux/amd64 与 linux/arm64 静态制品，并按目标机架构自动选择。
- 新增 MySQL 重启检测与分类，可区分平台内手动重启和意外重启；邮件告警明确支持 SMTP 465 SSL 与 587 STARTTLS。
- 完善 `server_id` 实例级管理、PT 离线依赖包校验、跨发行版安装、软件包兼容性检查和任务中心文档。

Bug 修复：

- 修复 Agent/MySQL 安装前未严格校验发行版、systemd、CPU 架构和 glibc，导致不兼容制品可能被下发的问题。
- 修复 Agent systemd 服务只启动但未可靠启用开机自启，以及安装后未同时验证 `is-enabled`/`is-active` 的问题。
- 修复 MGR 活跃成员仍可被重命名、删除或移出集群，以及首次转换前错误要求 Group Replication 插件已加载的问题。
- 修复 MGR 恢复账号轮换未要求所有成员在线、MySQL Shell 与 Server 系列不匹配及部分 AdminAPI 操作使用强制模式的问题。
- 修复任务列表状态落后于详情、失败安装续跑会重复成功步骤、运行中/子任务可能被错误跳过，以及恢复重试覆盖原失败记录的问题。
- 修复集群内 `server_id` 重复、修改后实例清单未同步，以及架构规划未自动修复冲突值的问题。
- 修复 SMTP 端口与加密方式不匹配、中文邮件主题编码不正确、地址校验不足和连接错误提示不明确的问题。
- 修复 MySQL 重启告警无法区分平台手动操作与异常重启，以及重复/乱序采样可能产生误报的问题。
- 修复 PT 离线包仅凭文件名判断依赖完整性、旧发行版依赖包名选择错误和任务日志可能泄露敏感命令内容的问题。

### V0.0.4（2026-07-27）

更新内容：

- AI 运维新增服务端模糊意图解析，可按正序/倒序序号、名称、IP 和经过验证的上下文代词定位机器或集群；即使模型未返回计划，也可基于当前平台对象生成受控计划。
- AI 会话升级为结构化滚动记忆，只携带最多 8 条近期消息，并同时限制字符数、Token 预算和单次输入大小；能力接口会公开这些上下文边界。
- 新增机器删除受控动作：默认仅从 GMHA 脱管，远端清理必须由用户明确表达；集群成员、依赖关系和审批条件由服务端预检。
- 补充 AI 上下文压缩、敏感信息清洗、模糊目标解析、机器删除安全边界等自动化测试和 API 文档。

Bug 修复：

- 修复长会话反复发送完整历史、超长单条消息或旧版记忆可能突破上下文窗口的问题。
- 修复历史消息中的密码、令牌、API Key 或私钥可能进入会话摘要和模型上下文的问题。
- 修复序号越界、目标重名或代词指向不明确时可能选错机器/集群的问题；现在会拒绝歧义目标。
- 修复机器脱管被意外扩大为远端卸载/清理、集群成员未移除即可删除，以及仅脱管仍错误要求 Agent 在线的问题。
- 修复已结束计划仍残留在会话“活动意图”中，影响后续指代解析的问题。

### V0.0.3（2026-07-24）

更新内容：

- AI 运维新增持久化会话、新建会话、会话选择、归档与恢复。
- 增加滚动摘要、服务端验证的活动意图、工作流检查点和执行恢复能力。
- 完善 AI 会话控制台、计划审批区域、能力发现接口、安全说明和测试覆盖。

Bug 修复：

- 修复高风险计划可能绕过确认、过期计划状态不清晰以及点击事件被误当作执行计划的问题。
- 修复不同会话上下文混用、VIP 追问丢失已确认参数或使用非规范对象 ID 的问题。
- 修复会话记忆允许保存敏感赋值，以及上下文压缩可能删除持久化会话记录的问题。
- 修复输入法组合输入、Enter 发送和多行输入之间的交互冲突。

### V0.0.2（2026-07-23）

更新内容：

- 新增 AI 运维工作流、性能监控、SQL 诊断、执行计划与 Binlog 分析。
- 新增索引、直方图、在线 DDL、数据归档、备份恢复和火焰图管理。
- 新增集群滚动升级、Manager 高可用、数据库巡检以及完整的控制台/API 文档。
- 扩展告警、拓扑、心跳历史、任务跟踪和离线工具包能力。

Bug 修复：

- 修复 MySQL 5.7 拓扑识别、配置模板和跨版本兼容判断问题。
- 修复主机内存指标、动态采集配置合并和性能样本统计不准确的问题。
- 修复长任务输出截断、任务进度展示及失败状态持久化不一致的问题。
- 修复 MySQL 安装/卸载、PT 工具准备、Agent 升级和软件包选择中的边界条件。
- 修复备份恢复预检、目标识别、恢复脚本参数和告警/心跳历史持久化问题。

### V0.0.1（2026-07-18）

更新内容：

- 首个版本化发布，提供 Manager、Agent、Web 启动器和 Linux amd64 完整安装包。
- 提供机器纳管、Agent 部署、MySQL 安装/卸载、集群拓扑、VIP、心跳监控和故障转移能力。
- 提供 Manager/Agent 在线升级、MySQL 用户管理、任务跟踪和 Web 控制台。

Bug 修复：

- 修复发布包校验文件在不同 Linux/macOS 工具环境下的兼容性。
- 修复 Manager/Agent 版本未正确嵌入、版本格式不统一和 Agent 无法直接查询版本的问题。
- 修复 SSH 执行、任务状态落库、心跳历史记录及部分安装卸载流程的稳定性问题。
- 修复前端构建产物引用、机器批量操作和实例管理中的若干状态同步问题。
...
构建结果位于 `dist/gmha-V0.3.2-linux-amd64.tar.gz`，并同时生成 SHA-256 校验文件。
